...
object Huffman {
    ...
    def times(chars: List[Char]): List[(Char, Int)] = {
        def loop(acc: Map[Char, Int], remainingChars: List[Char]): Map[Char, Int] = {
            if (remainingChars.isEmpty) acc
            else {
                val count = acc.get(remainingChars.head).getOrElse(0) + 1
                loop(acc + ((remainingChars.head, count)), remainingChars.tail)
            }
        }

        loop(new HashMap[Char, Int](), chars).toList
    }
    ...
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
        freqs.toStream.map(f => Leaf(f._1, f._2)).sortBy(l => l.weight).toList
    }
    ...
    def combine(trees: List[CodeTree]): List[CodeTree] = {
        if (singleton(trees)) trees
            ...
            trees.tail.tail.::(fork).sortBy(c => weight(c))
        }
    }
